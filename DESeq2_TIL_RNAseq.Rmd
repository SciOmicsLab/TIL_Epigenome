...
count_data <- read.table("../../data/rna_counts.csv",header = TRUE, row.names=1, sep = ",") 
...
title_supp <- ""
plt_pca1_cd4_pv <- ggplot(as.data.frame(pca_results_cd4$x), 
                          aes(x=PC1, y=PC2, label=rownames(as.data.frame(pca_results_cd4$x)))) +
...
plt_pca2_cd4_pv <- ggplot(as.data.frame(pca_results_cd4$x), 
                          aes(x=PC2, y=PC3, label=rownames(as.data.frame(pca_results_cd4$x)))) +
...
plt_pca1_cd8_pv <- ggplot(as.data.frame(pca_results_cd8$x), 
                          aes(x=PC1, y=PC2, label=rownames(as.data.frame(pca_results_cd4$x)))) +
...
plt_pca2_cd8_pv <- ggplot(as.data.frame(pca_results_cd8$x), 
                          aes(x=PC2, y=PC3, label=rownames(as.data.frame(pca_results_cd4$x)))) +
      geom_point(aes(color = clinical_data_cd8$TILlowhi_CD8), size = 3) +
      scale_color_manual(values = c("blue", "red", "green")) +
      geom_text_repel() + 
