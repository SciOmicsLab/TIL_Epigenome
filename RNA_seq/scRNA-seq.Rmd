...
knitr::opts_chunk$set(root.dir = "/Users/brianthompson/Desktop/TIL_Epigenome_scRNAseq")
```
...
Read in patient files and meta data
```{r patient files}
...
TIL <- Reduce(function (...) { merge(..., all = TRUE, by = "Genes") },   # Full join
...
VlnPlot(TIL_object, features = c("nFeature_RNA", "nCount_RNA"), ncol = 2)
grep(glob2rx("*MKI67*"), rownames(GetAssayData(TIL_object, slot = "counts")), value = TRUE) #MT-
TIL_object[["percent.mt"]] <- PercentageFeatureSet(TIL_object, pattern = "^MT-")
TIL_object <- NormalizeData(TIL_object)
```

Custome function to generate the number of cells positive for a given gene

```{r cell positive function}
...
Subset CD4 cells and analyze

```{r CD4}
TIL_CD4 <- subset(TIL_object, subset = CD4 > 0 & CD8A < 1 & percent.mt < 20, slot = "data")

#This list has been curated to only those that appear in the scRNA-seq dataset
...
#####Put together one big faceted plot with t-tests for the validated genes
ultimate_join <- join(OSM_CD4, ETS2_CD4, by = "Patient")
ultimate_join <- join(ultimate_join, TNFRSF18_CD4, by = "Patient")
til_categories <- ultimate_join[10]
ultimate_join %<>% select(Patient, ETS2, TNFRSF18, OSM)
ultimate_join$TIL_cat <- til_categories
ultimate_join %<>% pivot_longer(cols = c(2:4), names_to = "Sample", 
                               values_to = "Counts")

  
base:::mean(filter(OSM, TIL_cat == "High")$OSM)#2.09
base:::mean(filter(OSM, TIL_cat == "Low")$OSM)#11.67
base:::mean(filter(ETS2, TIL_cat == "High")$ETS2)#3.13
base:::mean(filter(ETS2, TIL_cat == "Low")$ETS2)#10.53
base:::mean(filter(TNFRSF18, TIL_cat == "High")$TNFRSF18)#30.69
base:::mean(filter(TNFRSF18, TIL_cat == "Low")$TNFRSF18)#44.71



cell_counts_plot <- ggplot(ultimate_join, aes(Sample, Counts)) +
...
pdf("cell_counts_plot.pdf", height = 8, width = 8)
cell_counts_plot
dev.off()
...
pdf("CD4_Validation_heatmap.pdf", height = 8, width = 8)
...
pdf("CD8_validation_heatmap.pdf", height = 8, width = 8)
...
######TIL Infused vs. OS######
coxph_model_TIL_OS = coxph(Surv(OS_months, Censor) ~ Number_TIL_Infused, data = df) 

OS_vs_TIL_Plot = 
ggplot(df, aes(x=Number_TIL_Infused, y=OS_months, shape=as.factor(Censor))) +
  geom_point(size=3) +
  theme_classic() + 
  ...
  annotate("text", x = 2, y = 100, label = paste("R² =", round(summary(coxph_model_TIL_OS)$rsq[1], 2))) +
  annotate("text", x = 2, y = 95, label = paste("p =", round(summary(coxph_model_TIL_OS)$waldtest[3], 5)))

OS_vs_TIL_Plot

#####Percent CD4 vs. OS#####
coxph_model_CD4_OS = coxph(Surv(OS_months, Censor) ~ Percent_CD4, data = df) 

OS_vs_CD4_Plot = ggplot(df, aes(x=Percent_CD4, y=OS_months, shape=as.factor(Censor))) +
  geom_point(size=3) +
  theme_classic() + 
  labs(shape = "Censoring") +
...
CD4_vs_TIL_Plot = ggplot(df, aes(x=Number_TIL_Infused, y=Percent_CD4, color = median_OS)) +
  ...
  geom_smooth(
    ...
    linetype = "dashed", 
    color = "purple") +
  annotate("text", x = 1e9, y = 10, label = paste("R² =", round(summary(model)$r.squared, 2))) +
  annotate("text", x = 1e9, y = 6, label = paste("p =", round(summary(model)$coefficients[2,4], 6))) +
  scale_color_manual(values = c("blue", "red")) +
  # ylim(0,1) +
  # xlim(0,12) +
  xlab(label = "Number of TIL Infused") +
...
```{r Survival}
TIL_Survival_Plot = ggsurvplot(
  fit = survfit(Surv(OS_months, Censor) ~ TIL_High_Low, data = df),
  xlab = "Months",
